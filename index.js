...
async function submitEdit45Task({ images, prompt, size }) {
  const resp = await fetch('https://api.wavespeed.ai/api/v3/bytedance/seedream-v4.5/edit', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${WAVESPEED_API_KEY}` },
    body: JSON.stringify({
      enable_base64_output: false,
...
/* === SEEDREAM 4.5 EDIT === */
app.get('/v1/automations/webhookSeedance45EditGen', async (req, res) => {
  const baseId = req.query.baseId;
  const recordId = req.query.recordId;
  const tableIdOrName = req.query.tableIdOrName || 'IMG GEN';
  const fieldName = req.query.fieldName || 'Attachments';
  ...
  console.log('[seedance4.5v-edit] received:', { baseId, recordId, tableIdOrName, fieldName });
  ...
  try {
    ...
    const submitPromises = Array.from({ length: desired }, () => submitEdit45Task({ images: inputUrls, prompt, size }));
    const taskIds = await Promise.all(submitPromises);
    console.log(`[seedance4.5v-edit] submitting ${desired} tasks ->`, taskIds);
    ...
    console.log(`[seedance4.5v-edit] outputs: ${successes.length}/${desired} for record ${recordId}`);
    res.json({ ok: true, recordId, requested: desired, completed: successes.length, failed: failures.length });
  } catch (err) {
    console.error('[seedance4.5v-edit] ERROR:', err?.message || err);
    try {
      await patchAirtableRecord(baseId, tableIdOrName, recordId, {
        [errField]: String(err?.message || err),
        [statusField]: 'Error',
...
app.get('/v1/automations/wavespeedNanoBananaEdit', async (req, res) => {
  const baseId = req.query.baseId;
  const recordId = req.query.recordId;
  const tableIdOrName = req.query.tableIdOrName || 'tbllUoiAZq0eeVLJ0';
  const fieldName = req.query.fieldName || 'Attachments';
...
app.get('/v1/automations/wavespeedNanoBanana2Edit', async (req, res) => {
  const baseId = req.query.baseId;
  const recordId = req.query.recordId;
  const tableIdOrName = req.query.tableIdOrName || 'tblaexHtFag8IgtVU';
  const fieldName = req.query.fieldName || 'Attachments';
...
app.get('/v1/automations/wavespeedKling26Pro', async (req, res) => {
  const baseId = req.query.baseId;
  const recordId = req.query.recordId;
  const tableIdOrName = req.query.tableIdOrName || 'tblydCZ9nW2Uy5uMn';
  const fieldName = req.query.fieldName || 'generated_outputs';
